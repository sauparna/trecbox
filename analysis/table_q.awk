...
# pieces:
#
# testcol stem model qid map Rprec P_5 P_10
# t8 p tf_idf 401 0.1767 0.2104 0.4000 0.3667
#
...
END {
    printf("# %s %s %s %s %s %s %s %s", "testcol", "stem", "model", "qid", 
	   "map", "Rprec", "P_5", "P_10")
    for (k in s) {
	n = split(k, a, /\./)
	for (i=1; i<=n; i++)
